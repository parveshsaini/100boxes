...
const History = () => {
  ...
  const fetchHistory = async () => {
    const res = await axios.get(
      `${import.meta.env.VITE_API_URL}/api/v1/grid/history?page=${page}`
    );
  ...
  return (
    <div className="flex flex-col gap-6 px-6 py-8 bg-gray-800 text-white min-h-screen">
          ...
          {history.map((item) => (
            ...
            >
              ...
              <div className="mt-4 md:mt-0">
                <button
                  onClick={() =>
                    handleViewClick(item.state, item.row, item.col)
                  }
      ...
      <div className="flex justify-between items-center mt-6">
        ...
        <button
          className={` hover:bg-green-400 px-6 py-2 rounded-md transition duration-300 ${
